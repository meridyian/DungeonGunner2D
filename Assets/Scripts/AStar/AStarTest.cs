...
public class AStarTest : MonoBehaviour
{
    ...
    private Grid grid;
    // create the tilemap that will be duplicated
    ...
    private void StaticEventHandler_OnRoomChanged(RoomChangedEventArgs roomChangedEventArgs)
    {
        ...
        // will be used to duplicate the front tilemap
        SetUpPathTilemap();
    }
    ...
    {
        // if position is beyond the grid then return size
        if (position.x < instantiatedRoom.room.templateLowerBounds.x ||
            position.x > instantiatedRoom.room.templateUpperBounds.y)
        {
            return false;
        }
        else
